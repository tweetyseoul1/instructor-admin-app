...
교육신고서 / 수료증·자격증 신청서 / 관리자 확인페이지(운영자·강사 통합)를 **하나의 페이지(`index.html`)** 안에서
탭 클릭으로 전환하는 단일 페이지 웹앱입니다. 별도 페이지 이동 없이 로그인 상태에 따라 메뉴가 바뀝니다.
...
3. **Authentication > URL Configuration** 에서 아래를 실제 배포 도메인으로 등록합니다 (비밀번호 재설정 링크가 이 목록에 없으면 동작하지 않습니다). 한 페이지 앱이므로 루트 주소 하나만 등록하면 됩니다.
   - Site URL: `https://<배포도메인>`
   - Redirect URLs: `https://<배포도메인>/`
...
## 6. 페이지 구성 (모두 `index.html` 안의 탭)

단일 파일 `index.html`이 유일한 진입점입니다. 상단 탭을 클릭하면 페이지 이동 없이 화면만 전환됩니다.

| 탭 / 화면 | 설명 | 접근 | 로그인 상태일 때만 탭에 노출 |
|---|---|---|---|
| 수료증·자격증 신청 | 서식2: 수료증·자격증(1급/2급) 신청서 | 공개 (학생이 직접 제출) | 항상 노출 (기본 화면) |
| 교육신고서 | 서식1: 교육신고서 (교안 첨부, 제출 시 구글시트 동기화) | 강사/관리자 로그인 필요 | O |
| 관리자 확인페이지 | 서식3: 운영자는 전체 총괄, 강사는 본인이 담당한 신청만 | 로그인 필요 | O |
| 로그인 / 비밀번호 재설정 | 로그인 폼, 이메일로 재설정 링크 요청·처리 | 공개 | 로그아웃 상태일 때만 노출 |
...
- [ ] 로그아웃 상태에서 접속 시 "수료증·자격증 신청" 화면이 기본으로 보이는지 확인
- [ ] 로그인 없이 신청서 제출 — 수료증 / 1급 자격증 / 2급 자격증 각각 테스트, 자격증 선택 시에만 입금일자 입력란이 나타나는지 확인
- [ ] 관리자 계정으로 로그인(탭 이동 없이 같은 페이지에서 전환) → "관리자 확인페이지" 탭에서 "강사 계정 추가"로 강사 초대
- [ ] 초대 이메일 수신 → 링크로 비밀번호 설정 → 강사 계정 로그인
- [ ] 강사 계정으로 "교육신고서" 탭에서 제출 (PPT 교안 파일 첨부 포함) → 구글시트에 행이 추가되는지 확인
- [ ] 강사 계정으로 "관리자 확인페이지" 탭 → 본인이 담당한 신청 건만 신청종류별로 그룹핑되어 보이는지 확인
- [ ] 관리자 계정으로 "관리자 확인페이지" 탭 → 전체 신청 건이 보이고, 강사 이름 검색·신청종류 필터가 동작하는지 확인
- [ ] 관리자가 발급 상태를 "발급완료"로 토글 → 발급일자 자동 기록 확인
- [ ] 로그인 화면에서 "비밀번호를 잊으셨나요?" → 이메일 수신 → 링크 클릭 시 같은 페이지에서 비밀번호 재설정 화면으로 자동 전환되는지 확인
